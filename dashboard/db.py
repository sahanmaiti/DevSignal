# dashboard/db.py — full replacement
...
def clear_all_caches():
    """Call this to force a full data refresh."""
...
# TTL reduced to 60s so data refreshes every minute
...
def load_opportunities(min_score: int = 0,
    ...
    sql   = text(f"""
        SELECT
            ...
            date_found, updated_at, description_raw
        FROM opportunities
        WHERE {where}
...
def load_outreach_jobs() -> pd.DataFrame:
    """
    Loads jobs that have outreach messages.
    Separate function so the Outreach page always gets fresh data.
    """
    sql = text("""
        SELECT
            id, company, role, opportunity_score, outreach_message,
            recruiter_name, recruiter_role, linkedin_profile, email,
            apply_link, applied, date_found, job_source
        FROM opportunities
