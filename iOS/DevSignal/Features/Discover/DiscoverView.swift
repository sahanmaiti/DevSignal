...
struct DiscoverView: View {
    // Using StateObject with a factory closure so the ViewModel
    // can be replaced in tests via the init parameter below.
    ...
    init(api: (any APIClientProtocol)? = nil) {
        let client = api ?? APIClient.shared
        _viewModel = StateObject(wrappedValue: DiscoverViewModel(api: client))
    }
    ...
    private var jobList: some View {
        List {
            // ── Cache status banner ───────────────────────────────────────
            if let bannerText = viewModel.cacheStatus.bannerText {
                Section {
                    HStack(spacing: 8) {
                ...
                .listRowBackground(viewModel.cacheStatus.bannerColor)
                .listRowSeparator(.hidden)
            }
            
            // ── Error banner (server error while we have data) ────────────
