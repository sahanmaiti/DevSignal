...
final class APIClient {
    ...
    // Returns true if server is reachable AND key is valid
    // Calls /stats (a protected endpoint) instead of /health (public)
    func checkHealth() async -> Bool {
        do {
            let _: DashboardStats = try await fetch(path: "/stats")
            return true
        } catch APIError.unauthorized {
            return false  // server reachable but key is wrong
        } catch {
